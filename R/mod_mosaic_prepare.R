  ...
  fluidRow(
    ...
    col_9(
      conditionalPanel(
        ...
        plotOutput(ns("mosaic_plot"), height = "760px")
      ),
      conditionalPanel(
        condition = "input.showmosaic == 'mapview' & input.mosaiccrop != true", ns = ns,
        leafletOutput(ns("mosaic_mapview"), height = "760px")
      ),
      conditionalPanel(
        condition = "input.mosaiccrop == true", ns = ns,
        fluidRow(
          col_6(h3("Original mosaic"),
                editModUI(ns("mosaic_crop"), height = "760px")),
          col_6(h3("Cropped mosaic"),
                leafletOutput(ns("mosaiccropped"), height = "760px"))
        )
      )
...
mod_mosaic_prepare_server <- function(id, mosaic_data, r, g, b, re, nir, basemap) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$import_mosaic, {
      ...
      mosaic_data$mosaic <- terra::rast(input$import_mosaic$datapath)
